...
export interface Contact {
  ...
  account_id: string;
  /**
   * OJO: en la base (migracion 055) `phone` dejo de ser NOT NULL — un
   * contacto que llega por Instagram no tiene telefono, y el CHECK
   * `contacts_alcanzable` solo exige que haya `phone` o `ig_id`. Acá
   * sigue declarado como string porque hoy NINGUN camino crea un
   * contacto sin telefono (Instagram esta apagado en las dos cuentas) y
   * aflojarlo obliga a tocar ocho componentes de escritorio. Cuando se
   * prenda la ingesta de Instagram, esto pasa a `string | null` y esos
   * ocho lugares se revisan de una.
   */
  phone: string;
  ...
  /**
   * IGSID del contacto en Instagram (migracion 055). Null mientras el
   * contacto sea solo de WhatsApp. La persona es una sola: el mismo
   * contacto puede tener las dos conversaciones.
   */
  ig_id?: string | null;
  name?: string;
  email?: string;
...
/**
 * Canal por el que entra y sale una conversacion (migracion 055). El
 * canal vive en `conversations`, no en `messages`: el mensaje pertenece
 * a una conversacion y esa ya sabe por donde entro.
 */
export type Channel = 'whatsapp' | 'instagram';

/**
 * Que canales tiene prendidos una cuenta (`account_channels`, migracion
 * 055). La interfaz muestra el canal si hay fila y lo deja elegible si
 * `enabled`. Prenderlo es un UPDATE, no un deploy.
 */
...
export interface Conversation {
  ...
  /**
   * Migracion 055. En la base es NOT NULL DEFAULT 'whatsapp', asi que
   * toda fila leida lo trae. Opcional en el tipo para no romper los
   * objetos Conversation que se arman a mano en tests y en el
   * escritorio; quien lo lee cae a 'whatsapp' (`conversationChannel`
   * en src/lib/movil/canales.ts).
   */
  channel?: Channel;
